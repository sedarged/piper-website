...
| AQ-11 | The deployed A3 Snackville map download was a truncated PDF. Chrome displayed “Failed to load PDF document” and Poppler reported missing xref/trailer data. | Regenerated the map from the approved official artwork as a valid, one-page, full-bleed landscape A3 PDF with no added “official map” frame or background. The generator now embeds a high-quality optimized print stream that remains below the deployment transport ceiling. Added a structural regression test for every downloadable PDF. | Poppler validates the regenerated 1-page A3 file; a rendered proof confirms that the official map fills the full sheet, and the browser download is retested after deployment. |
