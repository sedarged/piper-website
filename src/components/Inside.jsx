...
/**
 * A horizontally scrollable rail of real pages from Book 1,
      ...
      <div className="wrap">
        <Reveal style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 18, flexWrap: "wrap", marginBottom: 30 }}>
          <div>
            ...
            <h2 className="h2 on-sky">Pages from book one</h2>
          </div>
      ...
      <div className="rail" ref={railRef} style={{ paddingLeft: "var(--pad)", paddingRight: "var(--pad)" }}>
        {SPREADS.map((s, i) => (
          <figure className="sp-c" key={i}>
            <div className="sp-i"><Img src={drive(s.img, 900)} alt={`Page ${s.page}. ${s.t}`} fb={`p.${s.page}`} /></div>
            <figcaption style={{
